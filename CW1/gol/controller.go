...
func client(p Params,c distributorChannels) {
	...
	//If p.Turns == 0, then there's no need to calculate the next state, so just send a FinalTurnComplete event.
	if p.Turns == 0 {
		var final []util.Cell
		for y := 0; y < p.ImageHeight; y++ {
			for x := 0; x < p.ImageWidth; x++ {
	...
	//create a request containing the original world and the parameters,
	//and a response to get back the final world from the engine.
	...
	//Calculate the alive cells, in order to send the FinalTurnComplete event.
	if p.Turns != 0 {
	var final []util.Cell
	for y := 0; y < p.ImageHeight; y++ {
		for x := 0; x < p.ImageWidth; x++ {
	...
	c.events <- ImageOutputComplete{CompletedTurns: p.Turns, Filename: filename1}
